...
function buildGanttData(issue) {
  ...
  if (transitions.length === 0) {
    ...
    const endDate = issue.fields.resolutiondate || (isDone(lastTransition.to) ? lastTransition.at : null);
    segments.push({
      status: lastTransition.to,
      start: lastTransition.at,
      end: endDate,
      durationMs: isDone(lastTransition.to) ? 0 : endDate
