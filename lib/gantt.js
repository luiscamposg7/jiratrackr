function buildGanttData(issue) {
  ...
  if (transitions.length === 0) {
    ...
    const lastStatus = (lastTransition.to || '').toLowerCase();
    const isDoneStatus = lastStatus.includes('finaliz') || lastStatus.includes('done') ||
      lastStatus.includes('listo') || lastStatus.includes('closed') ||
      lastStatus.includes('resolved') || lastStatus.includes('resuelto');
    const endDate = issue.fields.resolutiondate || (isDoneStatus ? lastTransition.at : null);
    segments.push({
      status: lastTransition.to,
      start: lastTransition.at,
      end: endDate,
      durationMs: isDoneStatus ? 0 : endDate
